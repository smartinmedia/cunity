...
<tr class="user-{-$userItem.groupid}">
                            <td>{-$i+1}</td>
                            <td><a href="{-"index.php?m=profile&action="|URL}{-$userItem.username}">{-$userItem.name}</a></td>
...
{-else if $userItem.groupid eq 0}
<label class="label label-default">{-"Not verified"|translate}</label>
{-else if $userItem.groupid eq 2}
<label class="label label-warning">{-"Administrator"|translate}</label>
{-else if $userItem.groupid eq 3}
<label class="label label-info">{-"Cunity-Owner"|translate}</label>
{-else if $userItem.groupid eq 4}
<label class="label label-danger">{-"Blocked"|translate}</label>
{-/if}
</td>
<td>
<div class="btn-group btn-group-sm">
<button type="button" class="btn btn-primary dropdown-toggle" {-if $user.userid eq $userItem.userid || $userItem.groupid eq 3} disabled{-/if} data-toggle="dropdown">{-"Actions"|translate}&nbsp;<span class="caret"></span></button>
<ul class="dropdown-menu" role="menu">
<li><a href="changeUserStatus('delete');"><i class="fa fa-trash-o"></i>&nbsp;{-"Delete this user"|translate}</a></li>
<li><a href="changeUserStatus('delete');"><i class="fa fa-ban"></i>&nbsp;{-"Block this user"|translate}</a></li>
{-if $user.groupid eq 0}
<li><a href="changeUserStatus('activate',1);"><i class="fa fa-check"></i>&nbsp;{-"Activate this user"|translate}</a></li>
{-/if}
{-if $userItem.groupid < 2}
<li class="divider"></li>
{-if $userItem.groupid == 1}
<li><a href="changeUserStatus('groupid',2);"><i class="fa fa-wrench"></i>&nbsp;{-"Make Administrator"|translate}</a></li>
{-else if $userItem.groupid eq 2}
<li><a href="changeUserStatus('groupid',1);"><i class="fa fa-ban"></i>&nbsp;{-"Remove Administrator"|translate}</a></li>
{-/if}
{-/if}
</ul>
</div>
