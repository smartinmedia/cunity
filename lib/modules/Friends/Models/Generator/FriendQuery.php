...
class FriendQuery
{
    ...
    public static function getFriendListQuery($status = "> 0")
    {
        return new Zend_Db_Expr(Cunity::get("db")->select()
            ->from(Cunity::get("config")->db->params->table_prefix . "relations", new Zend_Db_Expr("(CASE WHEN sender = " . $_SESSION['user']->userid . " THEN receiver WHEN receiver = " . $_SESSION['user']->userid . " THEN sender END)"))
            ->where("status" . $status)
