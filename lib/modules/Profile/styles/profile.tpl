...
<style>@media screen and (max-width: 1023px) {.profile-banner {background-image: url('{-$profile.timg}') !important}}</style>
<div class="profile-banner{-if $profile.timg eq null} profile-banner-empty{-/if}"
    ...
    {-if $user.userid != $profile.userid}
        <div class="nofriends-buttons btn-group friend-buttons{-if $profile.status !== null} hidden{-/if}">
            <button class="btn btn-default" data-userid="{-$profile.userid}" data-action="addasfriend" data-toggle="modal"
                    data-target="#relationship-modal"><span class="fa fa-plus"></span> {-"Add as friend"|translate}</button>
        ...
        <div class="request-buttons btn-group friend-buttons{-if $profile.status != 1 || $profile.sender != $profile.userid} hidden{-/if}">
            <button class="btn btn-default" data-userid="{-$profile.userid}" data-action="confirmfriend" data-toggle="modal"
        ...
        <div class="pending-buttons btn-group friend-buttons{-if $profile.status != 1 || $profile.sender == $profile.userid} hidden{-/if}">
            ...
            <ul class="dropdown-menu">
                ...
                <li><a href="#" data-userid="{-$profile.userid}" data-action="removerequest" data-toggle="modal"
                       data-target="#relationship-modal"><i class="fa fa-times"></i> {-"Remove request"|translate}</a></li>
            </ul>
        </div>
...
<ul class="nav nav-pills profile-menu" id="profile-menu">
    ...
    <li class=""><a href="#Photos" id="profile-menu-photos" data-toggle="pill" onclick="loadAlbums({-$profile.userid});"><i class="fa fa-picture-o"></i>&nbsp;{-"Photo Albums"|translate}&nbsp;<span class="badge">{-if $profile.albumscount > 0}{-$profile.albumscount}{-/if}</span></a></li>
        ...
        <li class="pull-right hidden-sm hidden-xs"><a href="{-"index.php?m=profile&action=edit"|URL}" id="profile-menu-edit" class="tab-no-follow"><i class="fa fa-wrench"></i>&nbsp;{-"Edit profile"|translate}</a></li>
        {-/if}
</ul>
<div class="tab-content" style="border-top:1px solid #ccc">
    {-if ($profile.privacy.visit eq 1 && $profile.status > 2) || $profile.privacy.visit eq 3 || $profile.userid eq $user.userid}
        <div class="profile-pins tab-pane fade in clearfix" id="Pins">
            <div class="alert alert-block alert-danger hidden"><p>{-"This user does not have any profile pins!"|translate}</p></div>
            ...
            <div class="newsfeed-container pull-left">
                <form class="newsfeed-newpost ajaxform clearfix" action="{-"index.php?m=newsfeed&action=send"|URL}" method="post">
                    ...
                    <div class="btn-group-vertical pull-left hidden newsfeed-post-option-buttons hidden-sm hidden-xs">        
                        <button class="btn btn-default" type="button" onclick="postText();" id="newsfeed-post-text-button"><span class="fa fa-pencil fa-fw"></span></button>
                        <button class="btn btn-default" type="button" onclick="postImage();" id="newsfeed-post-photo-button"><span class="fa fa-picture-o fa-fw"></span></button>
                    </div>
                ...
                <section title="upcoming events" style="min-height: 500px;">
                    ...
                    <div class="btn-group btn-group-justified" data-toggle="buttons">
                        ...
                        <label class="btn btn-default tooltip-trigger active" data-title="{-"Show photos"|translate}">
                            <input type="checkbox" class="newsfeed-filter" checked value="image"><i class="fa fa-fw fa-picture-o"></i>
                        </label>
        ...
        <div id="Friends" class="tab-pane fade in">
            <div id="friendslist">
                ...
                <div class="list clearfix"><div class="alert alert-block alert-danger hidden"><p>{-"There are no users to show"|translate}</p></div></div>
            </div>
            ...
            <div id="Photos" class="tab-pane fade in">
                ...
                <script type="text/html" id="albums-template">
                    <div class="albumlist-item pull-left user-album-{%=o.userid%}">
                        <a href="{%=convertUrl({'module':'gallery','action': o.id,'x':o.title.replace(' ','_')})%}" class="albumlist-item-image" style="background-image:url('{%=checkImage(o.filename,'gallery','thumb_')%}');"></a>
                        <a href="{%=convertUrl({'module':'gallery','action': o.id,'x':o.title.replace(' ','_')})%}" class="albumlist-item-name">
                            {% if (o.type == 'profile') { %}
                        ...
                        <div class="albumlist-item-ownerbox clearfix">
                            <a href="{-"index.php?m=profile&action="|URL}{%=o.username%}"><img src="{%=checkImage(o.pimg,'user','cr_')%}" class="albumlist-item-owner tooltip-trigger pull-left" data-title="{%=o.name%}" data-placement="right"></a>
