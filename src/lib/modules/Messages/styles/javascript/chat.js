var chatBoxes = new Object(), chatScrollApi = [], chatStatus = true, smileys = new Array(), currentChatPanel = 0;
...
function refreshStatus() {
    for (var i in chatBoxes) {
        ...
        if ((data.partners.length === 1 && typeof userData === "undefined") || userData.online === 0)
...
function openChatBox(data) {
    ...
    data.users = "", data.partners = new Array();
    for (var u in users) {
        var p = users[u].split('|');
        data.users += p[0];
...
$(document).ready(function () {
    $.getJSON(siteurl + "style/" + design + "/img/emoticons/emoticons.json", function (data) {
        ...
        if (cStorage.get("chatBoxes") === null)
            cStorage.set("chatBoxes", new Object());
        else {
            var boxes = cStorage.get("chatBoxes");
            for (var i in boxes)
                openChatBox(boxes[i]);
